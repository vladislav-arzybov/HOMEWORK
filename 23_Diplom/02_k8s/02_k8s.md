...
[Ссылка на репозиторий с конфигурационными файлами Terraform](https://github.com/vladislav-arzybov/HOMEWORK/tree/main/23_Diplom/02_k8s/02_infra)
...
> Дополнительно настроено получение статического ip для master-node
...
> Выполняем команды terraform plan, terraform apply, создаем 3 ВМ, дополнительно получаем информацию по ip адресам из outputs:

[outputs.tf](https://github.com/vladislav-arzybov/HOMEWORK/blob/main/23_Diplom/02_k8s/02_infra/outputs.tf)

<img width="519" height="348" alt="изображение" src="https://github.com/user-attachments/assets/166095cf-533b-4b22-b388-5ec9872bc239" />

> Также созданные ВМ можно посмотреть в интерфейсе yandex cloud

<img width="1339" height="337" alt="изображение" src="https://github.com/user-attachments/assets/17d835cd-2c05-4c6a-b5dd-483f8119a691" />

---

> Для удобства отладки и тестирования при создании ВМ через cloud-config прокидываются метаданные (уз, mc и редактор nano).

[cloud-init.yml](https://github.com/vladislav-arzybov/HOMEWORK/blob/main/23_Diplom/02_k8s/02_infra/cloud-init.yml)
```
#cloud-config
users:
  - name: ${username}
...
> При выполнении дипломной работы были внесены изменения в блок объявления переменных для cloud-init.yml,  ```ssh_public_key = chomp(var.ssh_public_key)``` - chomp необходим для автоматического удаления пустой строки в конце переменных, особенность Kubernetes secret и Atlantis, в противном случае при выполнинии apply на terraform локально и через atlantis фиксировались расхождения(diff) в блоке metadata.
...
- [create_inventory_file.tf](https://github.com/vladislav-arzybov/HOMEWORK/blob/main/23_Diplom/02_k8s/02_infra/create_inventory_file.tf)
- [inventory.tftpl](https://github.com/vladislav-arzybov/HOMEWORK/blob/main/23_Diplom/02_k8s/02_infra/inventory.tftpl)
```
...
<img width="580" height="419" alt="изображение" src="https://github.com/user-attachments/assets/4e2fd753-7456-43c1-8eb0-5192afa8e41a" />

---
---

#### Настройка Kubernetes-кластера через Kubespray 

[Ссылка на репозиторий с конфигурацией Ansible](https://github.com/vladislav-arzybov/HOMEWORK/tree/main/23_Diplom/02_k8s/02_infra/ansible)
...
> В процессе выполнения плейбука ```site.yaml``` выполняется ряд задач по первичной настройке сервера:
...
- добавление внешнего IP-адреса в TLS-сертификат API-сервера Kubernetes (необходимо для доступа к кластеру с локального ПК)
...
<img width="984" height="182" alt="изображение" src="https://github.com/user-attachments/assets/83a54a73-e919-482f-822a-f795ae82bb5c" />
...
<img width="842" height="455" alt="изображение" src="https://github.com/user-attachments/assets/c8b2ff3f-1fdc-4963-9770-2a0dd23ac727" />
...
<img width="843" height="452" alt="изображение" src="https://github.com/user-attachments/assets/98b745c5-5b76-46dd-8abd-ce71ece47c80" />
