...
> После успешного выполнения первого этапа первичной настройки матстер ноды появится сообщение инструкцией по дальнейшим действиям на удаленном сервере, запуску kubespray.
...
> После установки кластера k8s можно вернуться в окно site.yaml и продолжить дальнейщую настройку мастер ноды, в этом блоке будут выполнены задания по:
- создании каталога .kube для текущего пользователя
- копирование kubeconfig из /etc/kubernetes/admin.conf 
- выдачей необходимых прав для работы с kubectl
- а также копирование kubeconfig на локальную машину с заменой ip адреса
...
> Также можно проверить доступность кластера с локальной машины на которой запускался terraform и ansible. Доступ есть, настройка выполнена успешно.
