...
Опция -sS запускает метод сканирования с помощью протокола TCP SYN, но полноценное TCP-соединение с портом сканируемой машины не устанавливается. В логе помимо скинирования портов и указания ip адрес атакующей машины мы также видим попытку подключения к удаленному рабочему столу.
